...
{
	if (OnFBoxLightBeginOverlap.IsBound())
...
DECLARE_Delegate_TwoParams(델리게이트 이름, 파라미터 타입1, 파라미터 타입2)
...
DECLARE_Delegate Dynamic SPARS_DELEGATE_TwoParams(델리게이트 이름, 소속 클래스명, 변수명,
파라미터 타입1, 파라미터 이름1, 파라미터 타입2,파라미터 이름2))



DECLARE_EVENT 다이나믹 델리게이트와 상당히 유사
